...
void SEC_read_debug(struct ufs_hba * hba, void __user *buffer);
#endif
...
#ifdef UFS_SEC_READ_DEBUG_INFO
	/*do not control if block_hibern8 is set*/
	if (hba->block_hibern8)
		goto rel_lock;
#endif
...
#ifdef UFS_SEC_READ_DEBUG_INFO
void SEC_read_debug_info(struct ufs_hba * hba, struct scsi_cmnd *cmd, u32 debug_buffer[16420])
{
	u32 vendor_flags;
	u32 vendor_flags_stored;
	u32 vendor_data;
	int tag = 0;
	unsigned long flags;
	struct ufshcd_lrb *lrbp;
	//struct utp_transfer_req_desc *req_desc_test;
	...
	int err;
// 1st
	err = ufshcd_dme_peer_get(hba, UIC_ARG_MIB(PA_VENDOR_FLAG),
			&vendor_flags);
	if (err) {
		printk("[UFS] %s:%d. err = %d.\n", __func__, __LINE__, err);
		return;
	}
	vendor_flags_stored = vendor_flags;
	printk("[UFS] %s:%d. lrb_in_use = 0x%lx. hba->outstanding_reqs = 0x%lx. vendor_flag=0x%08x.\n",
		__func__, __LINE__,	hba->lrb_in_use, hba->outstanding_reqs,	vendor_flags_stored);
...
// 2
	...
	wait_event(hba->dev_cmd.tag_wq, ufshcd_get_dev_cmd_tag(hba, &tag));
#endif
	printk("[UFS] %s:%d. lrb_in_use = 0x%lx.\n", __func__, __LINE__, hba->lrb_in_use);
	...
	lrbp->command_type = UTP_CMD_TYPE_DEV_MANAGE;
	lrbp->intr_cmd = true; /* No interrupt aggregation */
	lrbp->cmd->sc_data_direction = DMA_NONE;

	printk("[UFS] %s:%d. tag = %d.\n", __func__, __LINE__, tag);

	// Compose UPIU as SCSI cmd
	ufshcd_prepare_req_desc_hdr(lrbp, &upiu_flags,
			lrbp->cmd->sc_data_direction);
	ufshcd_prepare_utp_scsi_cmd_upiu(lrbp, upiu_flags);
	...
	init_completion(&wait);
		hba->dev_cmd.complete = &wait;
		printk("UPIU Header: dword0: 0x%x, dword1: 0x%x, dword2: 0x%x\n",
			lrbp->ucd_req_ptr->header.dword_0, lrbp->ucd_req_ptr->header.dword_1, lrbp->ucd_req_ptr->header.dword_2);
		/* Make sure descriptors are ready before ringing the doorbell */
		spin_lock_irqsave(hba->host->host_lock, flags);
		
		// ISTOR SPECIFIC CHANGES - START
		if (hba->vops && hba->vops->set_nexus_t_xfer_req)
			hba->vops->set_nexus_t_xfer_req(hba,
					tag, lrbp->cmd);
		// ISTOR SPECIFIC CHANGES - END
		...
		printk("[UFS] %s:%d. lrb_in_use = 0x%lx. hba->outstanding_reqs = 0x%lx.\n", __func__, __LINE__,
		...
		do {
			...
			if (i > 1000) {
				printk("[UFS] %s:%d. lrb_in_use = 0x%lx. hba->outstanding_reqs = 0x%lx."
						...
						hba->lrb_in_use, hba->outstanding_reqs,
						vendor_flags);
				vendor_flags_stored = ~vendor_flags_stored;
	//			break;
			} else
				msleep(1);
			i++;
		} while (vendor_flags_stored == vendor_flags);
	
	// 4
		printk("[UFS] %s:%d.\n", __func__, __LINE__);
		err = ufshcd_dme_peer_set(hba,
				UIC_ARG_MIB(PA_VENDOR_FLAG), vendor_flags_stored);
		if (err) {
			printk("[UFS] %s:%d. err = %d.\n", __func__, __LINE__, err);
			return;
		}
	...
	// 5
		...
		printk("Initial value: Sense Key: %d, ASC: 0x%x, ASCQ: 0x%x\n",
			(lrbp->sense_buffer[2] & 0xf),lrbp->sense_buffer[12],lrbp->sense_buffer[13]);
		do {
			do {
				ufshcd_dme_peer_get(hba, UIC_ARG_MIB(PA_VENDOR_FLAG),
						&vendor_flags);
			} while (vendor_flags_stored == vendor_flags);
	
			ufshcd_dme_peer_get(hba, UIC_ARG_MIB(PA_VENDOR_DATA),
					&vendor_data);
			debug_buffer[temp_i] = vendor_data;
			//hba->debug_info_default[temp_val][i] = vendor_data;
			temp_i++;
			recv_data_size += 4;
			err = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_VENDOR_FLAG),
					vendor_flags_stored);
			if (err) {
				printk("[UFS] %s:%d. err = %d. temp_i = %d.\n", __func__, __LINE__, err, temp_i);
				return;
			}
		} while (recv_data_size < debug_buffer[0] + 4);
		// wait resp.
			printk("[UFS] %s:%d. lrb_in_use = 0x%lx. hba->outstanding_reqs = 0x%lx.\n", __func__, __LINE__,
					hba->lrb_in_use, hba->outstanding_reqs);
			printk("[UFS] %s:%d. Wait for response!!!\n", __func__, __LINE__);
			/* Make sure descriptors are ready before ringing the doorbell */
		
			//wait for completion
			time_left = wait_for_completion_timeout(hba->dev_cmd.complete,
					msecs_to_jiffies(QUERY_REQ_TIMEOUT * 10 * 10));
			...
			if (likely(time_left)) {
				...
				if (err == 2)
					printk("Sense Key: %d, ASC: 0x%x, ASCQ: 0x%x\n",
						(lrbp->sense_buffer[2] & 0xf),lrbp->sense_buffer[12],lrbp->sense_buffer[13]);
			} else {
				err = -ETIMEDOUT;
			}
			...
			printk("------------------>>>>>>>>>Recieved data size: %d\n", recv_data_size);
			printk("------------------>>>>>>>>>Recieved 4 byte count: %d\n", temp_i);
			printk("Current Value: %d\n", debug_buffer[0]);
			//print values
		
			for (i = 0; i < 16; i++) {
					printk("Index i: %lu, value: 0x%x\n", i,debug_buffer[i]);
				}
		
		
			if (err) {
				printk("[UFS] %s:%d. VENDOR REQUEST FAILED err = %d.\n", __func__, __LINE__, err);
			} else {
				printk("[UFS] %s:%d. VENDOR REQUEST SUCCESSFUL \n", __func__, __LINE__);
			}
			{
				unsigned long completed_reqs;
				u32 tr_doorbell;
				...
				tr_doorbell = ufshcd_readl(hba, REG_UTP_TRANSFER_REQ_DOOR_BELL);
				completed_reqs = tr_doorbell ^ hba->outstanding_reqs;
				printk("[UFS] %s:%d. tr_doorbell=0x%08x, hba->outstanding_reqs=0x%lx, completed_reqs=0x%lx.\n",
					__func__, __LINE__, tr_doorbell, hba->outstanding_reqs, completed_reqs);
...
void SEC_read_debug(struct ufs_hba * hba, void __user *buffer)
{
	...
	printk("[UFS] %s:%d. wait for clear... DOORBELL\n", __func__, __LINE__);
	for (pos = 0; pos < 16; pos++) {
		err = ufshcd_clear_cmd(hba, pos);
		if (err) {
			printk("[UFS] failed to clear tag lineno: %d\n", __LINE__);
			break;
		}
	}
	...
	printk("[UFS] Changing To Slow Mode, Gear 4 %s:%d.\n", __func__, __LINE__);

	err = ufshcd_uic_change_pwr_mode(hba, SLOW_MODE << 4 | SLOW_MODE);
	if (err) {
		dev_err(hba->dev,
			"Power mode change: %d\n", err);
		return;
	...
	printk("[UFS] Setting Lanes %s:%d.\n", __func__, __LINE__);
	/* Set the count for lane */
	ufshcd_dme_set(hba, UIC_ARG_MIB(PA_ACTIVERXDATALANES), 0);
	ufshcd_dme_set(hba, UIC_ARG_MIB(PA_ACTIVETXDATALANES), 0);

	printk("[UFS] Setting Gears %s:%d.\n", __func__, __LINE__);
	/* Set the count for gear */
	ufshcd_dme_set(hba, UIC_ARG_MIB(PA_RXGEAR), 4);
	ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXGEAR), 4);

	// START MARK
	memset(cmd.cmnd, 0, 16);
	cmd.cmnd[0] = 0xc1;
	cmd.sense_buffer = kzalloc(SCSI_SENSE_BUFFERSIZE, GFP_KERNEL);
	cmd.scsi_done = NULL;

	cmd.sdb.table.nents = 0;

	SEC_read_debug_info(hba, &cmd, hba->debug_info_default);

	mdelay(5000);
	memset(cmd.cmnd, 0, 16);
	cmd.cmnd[0] = 0xc1;
	cmd.cmnd[2] = 0x01;
	SEC_read_debug_info(hba, &cmd, hba->debug_info_extra);
	// END MARK

	panic("[UFS] DEVICE ERROR!.. check the RAMDUMP\n");
#if 0
	async_schedule(ufshcd_async_scan, hba);
#endif

	printk("[UFS] Set back to High Speed %s:%d.\n", __func__, __LINE__);
	err = ufshcd_uic_change_pwr_mode(hba, FAST_MODE << 4 | FAST_MODE);
	if (err) {
		dev_err(hba->dev,
			"Power mode change: %d \n", err);
		return;
	...
	/* Set the count for lane */
	ufshcd_dme_set(hba, UIC_ARG_MIB(PA_ACTIVERXDATALANES), 2);
	ufshcd_dme_set(hba, UIC_ARG_MIB(PA_ACTIVETXDATALANES), 2);

	/* Set the count for gear */
	ufshcd_dme_set(hba, UIC_ARG_MIB(PA_RXGEAR), 2);
	ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXGEAR), 2);
	hba->block_hibern8 = false;
	scsi_unblock_requests(hba->host);
	ufshcd_release(hba);
	printk("[UFS] %s: done/......\n", __func__);
