...
int bq51221_set_cc_cv_info(struct i2c_client *client)
{
	u8 data = 0;
	int ret = 0;
	int retry_cnt =0;
...
void bq51221_wireless_chg_init(struct i2c_client *client)
{
	...
	if((value.intval < charger->pdata->wireless_cc_cv) && (charger->pdata->pad_mode == BQ51221_PAD_MODE_WPC)) {
		/* init VOREG set 5.5V*/
		bq51221_reg_write(client, BQ51221_REG_CURRENT_REGISTER, 0x02);
		data = bq51221_reg_read(client, BQ51221_REG_CURRENT_REGISTER);
		pr_info("%s VOREG = 0x%x, SoC(%d)\n", __func__, data, value.intval);
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CHARGE_TYPE:
			if(val->intval) {
				bq51221_set_cc_cv_info(charger->client);
			}
		break;
		case POWER_SUPPLY_PROP_HEALTH:
